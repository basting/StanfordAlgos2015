# @author Philip Ronan (from class "Algorithms: Design and Analysis, Part 1" by Tim Roughgarden)
# uses Python 3.x. To use Python 2.x, replace input() with raw_input()
	#sample execution below::
	######################################
	#python create_test_data.py 
	#How many SCCs? 3
	#How many additional nodes? 7
	#Graph created. SCC sizes are as follows:
	#5,3,2
	#Output filename: test_data.txt
	#Finished
	######################################
...
def build_graph():
  global max_node, sccs, edges
  while True:
    nscc = int(input('How many SCCs? '))
    if nscc>=1:
      break
    print('Must have at least one. Try again.')
  while True:
    add_nodes = int(input('How many additional nodes? '))
    if add_nodes>=0:
      break
    print('No. Try again.')
